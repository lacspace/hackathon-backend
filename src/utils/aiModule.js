 ...
 */
export async function generatePatientReport(profileId, genes) {
    console.log('📝 Generating report for:', profileId);
    const highRiskGenes = genes.filter(g => g.phenotype.toLowerCase().includes('poor') ||
    ...
    const citations = ["CPIC Guidelines v4.2"];
    ...
    highRiskGenes.forEach(g => {
        Object.keys(advancedDb).forEach(drugName => {
            ...
            if (entry.genes.includes(g.gene)) {
                const evidence = entry.clinicalVariants.find((v) => v.gene === g.gene);
