 ...
 */
export async function processGenomicData(basePath) {
    const results = {};
    const drugToGenes = {};
    const drugToVariants = {};
    const drugToLabels = {};
    const drugToGuidelines = {};
    console.log('🧬 Starting Genomic Knowledge Ingestion...');
    // Helper to parse TSV lines
    const parseTSV = (filePath, callback) => {
    ...
    parseTSV(path.join(basePath, 'relationships/relationships.tsv'), (data) => {
        if (data.Entity1_type === 'Gene' && data.Entity2_type === 'Chemical') {
            ...
            if (!drugToGenes[chemical])
                drugToGenes[chemical] = new Set();
    ...
    parseTSV(path.join(basePath, 'clinicalVariants/clinicalVariants.tsv'), (data) => {
        const drugs = data.chemicals?.toLowerCase().split(',').map((s) => s.trim()) || [];
        drugs.forEach((drug) => {
            if (!drugToVariants[drug])
                drugToVariants[drug] = [];
